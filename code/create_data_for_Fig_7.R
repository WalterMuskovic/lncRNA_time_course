### Fig.7 explores the relationship between genomic distance, correlation and
### timing of protein-coding gene/lncRNA expression for mouse genes
...
# Import data
counts <- readRDS("data/mouse_filtered_coords.rds")
...
# Save out
saveRDS(c_c, "data/c_c_mouse.rds")
saveRDS(nc_nc, "data/nc_nc_mouse.rds")
saveRDS(c_nc, "data/c_nc_mouse.rds")

# If not already done, randomly permute genomic distance between coding
# transcripts 1000 times
if (!file.exists("data/c_c_percentiles_mouse.rds")) {
  ...
  saveRDS(naive_permute_c_c_results, "data/c_c_percentiles_mouse.rds")
}
...
if(!file.exists("data/nc_nc_percentiles.rds")) {
  ...
  # Save out, as takes a while
  saveRDS(naive_permute_nc_nc_results, "data/nc_nc_percentiles_mouse.rds")
}
...
#s(log10_dist) 6.704  7.814 36.39  <2e-16
quantiles <- readRDS("data/nc_nc_percentiles_mouse.rds")
...
#Approximate significance of smooth terms:
#  edf Ref.df     F p-value
#s(log10_dist) 8.343  8.881 93.99  <2e-16
# Get the 1st, 5th, 25th, 75th, 95th and 99th percentiles
quantiles <- readRDS("data/c_c_percentiles_mouse.rds")
...
#Approximate significance of smooth terms:
#  edf Ref.df     F p-value
#s(log10_dist) 7.007  8.077 32.21  <2e-16
...
chromosome_length <- c(chr1=195471971, chr10=130694993, chr11=122082543,
                       ...
                       chr18=90702639, chr19=61431566, chr2=182113224,
                       chr3=160039680, chr4=156508116, chr5=151834684,
                       chr6=149736546, chr7=145441459, chr8=129401213,
                       chr9=124595110, chrX=171031299, chrY=91744698)
...
if(!file.exists("data/block_bootstrap_quantiles_mouse.rds")){
  ...
  # Save out, as takes a while
  saveRDS(quantiles, "data/block_bootstrap_quantiles_mouse.rds")
  # Clean up
...
# Novel vs annotated mouse lncRNAs ----------------------------------------
...
saveRDS(c_nc, "data/c_nc_anno_mouse.rds")
if(!file.exists("data/S4_overlapping_block_bootstrap_quantiles_mouse.rds")){
  # Load furrr package to speed up running the block bootstrap
  plan(multisession, workers = 20)
  # Set how many times the bootstrap should be run
  n=1000
  ...
  quantiles <- apply(block_bootstrap_results , 2, quantile, probs = c(0.01,0.05,0.25,0.75,0.95,0.99) , na.rm = TRUE )
  # Save out, as takes a while
  saveRDS(quantiles, "data/S4_overlapping_block_bootstrap_quantiles_mouse.rds")
  # Clean up
  rm(n, quantiles, block_bootstrap_results)
}
...
saveRDS(c_nc, "data/c_nc_no_anno_mouse.rds")
if(!file.exists("data/S4_not_overlapping_block_bootstrap_quantiles_mouse.rds")){
  # Load furrr package to speed up running the block bootstrap
  plan(multisession, workers = 20)
  # Set how many times the bootstrap should be run
  n=1000
  ...
  quantiles <- apply(block_bootstrap_results , 2, quantile, probs = c(0.01,0.05,0.25,0.75,0.95,0.99) , na.rm = TRUE )
  # Save out, as takes a while
  saveRDS(quantiles, "data/S4_not_overlapping_block_bootstrap_quantiles_mouse.rds")
  # Clean up
...
if(!file.exists("data/c_nc_lagged_corr_mouse.rds")){
  ...
  c_nc <- readRDS("data/c_nc_mouse.rds") %>% dplyr::select(c("row_vec", "col_vec"))
  ...
  get_lag_cor <- function(x, coding_expression_type = "pre_mRNA"){
    ...
    if(coding_expression_type=="pre_mRNA"){
      return(as.numeric(ccf(x = unlist(c_nc$ncRNA[[x]]), y = unlist(c_nc$pre_mRNA[[x]]), lag.max = 6, type="correlation", na.action = na.omit, plot = FALSE)$acf))
    } else {
      return(as.numeric(ccf(x = unlist(c_nc$ncRNA[[x]]), y = unlist(c_nc$mRNA[[x]]), lag.max = 6, type="correlation", na.action = na.omit, plot = FALSE)$acf))
    }
  }
  ...
  colnames(lagged_cor_pre_mRNA) <- str_glue('premRNA_lag_{seq(-90,90,15)}')
  ...
  colnames(lagged_cor_mRNA) <- str_glue('lag_mRNA_{seq(-90,90,15)}')
  
  # Add to c_nc data frame and save out
  c_nc <- cbind(c_nc, lagged_cor_pre_mRNA)
  c_nc <- cbind(c_nc, lagged_cor_mRNA)
  saveRDS(dplyr::select(c_nc, -ncRNA, -pre_mRNA, -mRNA), file = "data/c_nc_lagged_corr_mouse.rds")
...
if(!file.exists("data/lagged_cor_bbstrap_mouse.rds")){
  ...
  # Save out
  saveRDS(lagged_cor_bbstrap, "data/lagged_cor_bbstrap_mouse.rds")

  # Clean up
  rm(lagged_cor, lag_bbstrap, lagged_cor_bbstrap, n)
}

print("Finished creating data for Fig. 7")
